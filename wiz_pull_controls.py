...
WIZ_CONFIG_PATH = "/Users/don/VSCode/Boto3Test/wizcreds.json"
# File extension and timestamp are automatically appended
# Final output filename will look like:
# wiz_controls-2023-11-01T21:12:57.305002Z.csv
CSV_FNAME = "wiz_controls"
############### End Script settings ###############
...
cloud_controls_query = """
  query ManageControlsTable($first: Int, $after: String, $filterBy: ControlFilters, $issueAnalyticsSelection: ControlIssueAnalyticsSelection) {
...
cloud_controls_query_vars = {
    "first": 500,
    "filterBy": {"withIssues": None},
    "quick": False,
}
...
def get_api_result(client: WizClient) -> pd.DataFrame:
    ...
    # Initliaze a timer
    func_time = Timer("+ Fetching Controls from Wiz")

    with yaspin(text=func_time, color="white"):
        # Query the wiz API
        result = query_wiz_api(
            client=client,
            query=cloud_controls_query,
            variables=cloud_controls_query_vars,
        )
        ...
        while page_info["hasNextPage"]:
            ...
            # Advance the cursor
            cloud_controls_query_vars["after"] = page_info["endCursor"]
